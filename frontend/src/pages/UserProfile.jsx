...
import { ArrowLeft, Save, User, Mail, ShieldCheck, BadgeInfo, Upload } from 'lucide-react';
...
const UserProfile = () => {
    ...
    const hasIdentityDocument = Boolean(identityDocumentFile || profile?.identity_document);
    const needsIdentityHint = form.role === 'civitas' && !form.identity_number.trim() && !hasIdentityDocument;
    ...
    if (loading) {
        return <div style={{ padding: '40px', color: '#6b7280' }}>Loading profile...</div>;
    }
    ...
    if (error && !profile) {
        return (
            <div style={{ padding: '40px' }}>
                ...
                <div style={{ padding: '24px', background: 'white', borderRadius: '18px', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.06)' }}>
        ...
        >
            <div style={{ maxWidth: '980px', margin: '0 auto' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '24px', alignItems: 'start' }}>
                    <form onSubmit={handleSubmit} style={{ background: 'white', borderRadius: '24px', padding: '28px', boxShadow: '0 18px 40px rgba(15, 23, 42, 0.08)' }}>
                        <div style={{ marginBottom: '24px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                                <div style={{ width: '48px', height: '48px', borderRadius: '16px', background: '#e0f2fe', color: '#0369a1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <User size={22} />
                                </div>
                                <div>
                                    <h1 style={{ margin: 0, fontSize: '28px', color: '#111827' }}>{isSelfRoute ? 'My Profile' : 'User Profile'}</h1>
                                    <p style={{ margin: '4px 0 0', color: '#6b7280' }}>Edit identity data here. Admin can also manage role and active status.</p>
                                </div>
                            </div>
                        </div>

                        {error ? (
                            <div style={{ marginBottom: '16px', padding: '12px 14px', borderRadius: '12px', background: '#fef2f2', color: '#b91c1c', fontWeight: '600' }}>
                                {error}
                            </div>
                        ) : null}

                        {success ? (
                            <div style={{ marginBottom: '16px', padding: '12px 14px', borderRadius: '12px', background: '#ecfdf5', color: '#047857', fontWeight: '600' }}>
                                {success}
                            </div>
                        ) : null}
                        ...
                        <div style={{ display: 'grid', gap: '18px' }}>
                            <label style={{ display: 'grid', gap: '8px' }}>
                                <span style={{ fontSize: '14px', fontWeight: '700', color: '#374151' }}>Full Name</span>
                                <input
                                    value={form.fullname}
                                ...
                                />
                                <span style={{ fontSize: '12px', color: '#6b7280' }}>Full name harus diisi.</span>
                            </label>
                            ...
                            <label style={{ display: 'grid', gap: '8px' }}>
                                ...
                                <input
                                    ...
                                    onChange={(event) => handleChange('identity_number', event.target.value)}
                            ...
                            <label style={{ display: 'grid', gap: '8px' }}>
                                ...
                                {profile?.identity_document && isPreviewableDocument(profile.identity_document) ? (
                                    <div style={{ border: '1px solid #e5e7eb', borderRadius: '16px', overflow: 'hidden', background: '#f8fafc' }}>
                                        <img
                                ...
                                >
                                    <div>
                                        <div style={{ fontSize: '14px', fontWeight: '700', color: '#111827' }}>
                                            {identityDocumentFile ? identityDocumentFile.name : 'Choose document'}
                                        </div>
                                        <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>
                                            Current file: {getDocumentName()}
                                        </div>
                                    </div>
                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: '#0f172a', fontWeight: '700' }}>
                                        <Upload size={16} />
                                        Upload
                                    </span>
                                    <input
                            ...
                            {canEditAll ? (
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                                    <label style={{ display: 'grid', gap: '8px' }}>
                                        <span style={{ fontSize: '14px', fontWeight: '700', color: '#374151' }}>Role</span>
                                        <select
                                            value={form.role}
                                    ...
                                    <label style={{ display: 'grid', gap: '8px' }}>
                                        <span style={{ fontSize: '14px', fontWeight: '700', color: '#374151' }}>Active Status</span>
                                        <select
                    ...
                    <aside style={{ display: 'grid', gap: '16px' }}>
                        <div style={{ background: 'white', borderRadius: '24px', padding: '22px', boxShadow: '0 18px 40px rgba(15, 23, 42, 0.08)' }}>
...
const Row = ({ label, value, icon }) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', padding: '12px 14px', borderRadius: '14px', background: '#f8fafc' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#6b7280', fontSize: '14px', fontWeight: '600' }}>
            {icon}
            {label}
        </div>
        <div style={{ color: '#111827', fontSize: '14px', fontWeight: '700', textAlign: 'right' }}>{value}</div>
    </div>
);
